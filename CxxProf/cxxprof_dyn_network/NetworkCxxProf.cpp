...
namespace CxxProf
{
    ...
    void NetworkCxxProf::shutdown()
    {
        ...
        //Send the objects
		{
			...
			currentObject = currentObject ? 0 : 1;
			sendObjects_ = &networkObjects[currentObject];
			sendObjects();
		}
    }
    ...
    void NetworkCxxProf::sendObjects()
    {
		//Protect the SendObjects_
		{
			...
			int i = currentObject ? 0 : 1;
			networkObjects[i].Info = info_;
			storage_.storeResult(networkObjects[i]);
			networkObjects[i].clear();
		}
    }
